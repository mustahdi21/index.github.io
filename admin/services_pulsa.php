	...
	if (mysqli_num_rows($check_user) == 0) {
		...
		if (isset($_POST['delete'])) {
			...
			if (mysqli_num_rows($checkdb_service) == 0) {
				...
				$delete_user = mysqli_query($db, "DELETE FROM services_pulsa WHERE sid = '$post_sid'");
				if ($delete_user == TRUE) {
					$msg_type = "success";
...
<h6 class="card-header"><i class="mdi mdi-history"></i> Data Service</h6>
<div class="card-body">
											<?php 
											if ($msg_type == "success") {
											?>
...
if (isset($_GET['search'])) {
	$search = $_GET['search'];
	$query_list = "SELECT * FROM services_pulsa WHERE oprator LIKE '%$search%' ORDER BY pid ASC"; // edit
} else {
	$query_list = "SELECT * FROM services_pulsa ORDER BY pid ASC"; // edit
}
...
// end paging config
												...
												?>
													<tr>
														...
														<td align="center">
														<a href="<?php echo $cfg_baseurl; ?>admin/service_pulsa/detail.php?sid=<?php echo $data_show['sid']; ?>" class="btn btn-xs btn-info btn-rounded"><i class="fa fa-eye"></i></a>
														<a href="<?php echo $cfg_baseurl; ?>admin/service_pulsa/edit.php?sid=<?php echo $data_show['sid']; ?>" class="btn btn-xs btn-warning btn-rounded"><i class="fa fa-edit"></i></a>
														<a href="<?php echo $cfg_baseurl; ?>admin/service_pulsa/delete.php?sid=<?php echo $data_show['sid']; ?>" class="btn btn-xs btn-danger btn-rounded"><i class="fa fa-trash"></i></a>
														</td>
													</tr>
												<?php
												}
